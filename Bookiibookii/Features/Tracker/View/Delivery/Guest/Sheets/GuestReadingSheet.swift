...
// 안드 fragment_guest_reading_bottom_dialog.xml (GuestReadingBottomDialogFragment) 대응.
// GUEST_READING / GUEST_EXTENSION — 게스트 본인 독서 중.
struct GuestReadingSheet: View {
    let title: String
    let startDate: String
    let endDate: String
    /// 연장 가능 여부 (안드 canExtend = extensionCount < 1 && status in [GUEST_READING, GUEST_EXTENSION]).
