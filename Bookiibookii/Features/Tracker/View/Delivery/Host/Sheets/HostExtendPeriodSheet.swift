...
// 안드 fragment_host_extend_period_dialog.xml (HostExtendPeriodDialogFragment) 대응.
// 본인 독서 기간 연장 신청 다이얼로그. 일수 1~7 입력.
struct HostExtendPeriodSheet: View {
    @Binding var days: String                 // "" | "1"~"7"
    let originalEndDate: String               // "2025.12.25."
    let extendedEndDate: String               // 계산 결과 표시 (입력 0/공백이면 = 원본)
    let onClose: () -> Void
    let onCancel: () -> Void
    let onApply: () -> Void
