...
// MARK: - 생년월일 휠 피커 시트 (안드로이드 BirthdatePickerBottomSheet 대응 — 커스텀 3컬럼 휠)
...
// 안드로이드 WheelPickerColumn 대응 — 5줄 노출, 가운데 정렬 스냅 휠
private struct WheelPickerColumn: View {
    ...
    var body: some View {
        ScrollViewReader { proxy in
            ...
            // 가운데에 온 항목을 콘텐츠 오프셋으로 계산해 선택값 갱신.
            // contentMargins(inset)만큼 오프셋이 음수로 시작하므로 inset을 더해 보정한다.
